...
export default function CarPanel() {
    ...
    if(!carResponse) return <div>Car Panel is Loading...</div>
    ...
    return (
        <div>
            ...
            <div className="w-full text-xl font-medium">CompareList : {compareList.size}</div>
            {Array.from(compareList).map( (car) =><div>{car}</div>)}
            ...
            <input type = "text" placeholder="Please fill" className="block text-gray-900 text-sm rounded-md border p-2 m-2 bg-purple-50 ring-1 ring-inset ring-purple-400
